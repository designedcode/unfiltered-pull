...
import { categories, searchServices, servicePath, type CategorySlug } from "@/lib/services";

export function ServiceSearch({
  scopedCategory,
  placeholder = "Search readings, healing, love, forecasts…",
}: {
    ...
    <div className="sticky top-[72px] z-30 bg-ivory/95 py-3 backdrop-blur-md">
      ...
      {query.trim() ? (
        <div className="paper-card mt-3 max-h-80 overflow-auto rounded-2xl p-3">
          {results.length === 0 ? (
            <p className="px-2 py-3 text-sm text-ink/70">No matches. Try love, reiki, or year ahead.</p>
          ) : (
              ...
              {results.slice(0, 20).map((service) => (
                <li key={`${service.category}-${service.slug}`}>
                  <Link
                    ...
                    className="flex min-h-12 flex-col justify-center px-2 py-2"
                  >
                    ...
                    <span className="font-accent text-[10px] uppercase tracking-[0.16em] text-gold-deep">
                      {categories.find((c) => c.slug === service.category)?.title}
                    </span>
                  </Link>
                </li>
              ))}
